...
// ABOUTME: Returns Category entity or throws exception if not found.
...
final readonly class FindCategoryRunner
{
    ...
    {
        $category = $this->categoryRepository->find(Ulid::fromString($query->categoryId));

        if (null === $category) {
            throw new \InvalidArgumentException(
                sprintf('Category with ID "%s" not found.', $query->categoryId)
            );
        }
